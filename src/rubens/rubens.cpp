...
// ###########################################################################################
// ------                                                                               ------
// ------                        QoI recalculations called rubens                       ------
// ------                                                                               ------
// ###########################################################################################
class RubensWithKOm : public RubensWithModels, public RansTurbKOm {
protected:
	// Class name
	string classID;
...
public:
	 ...
	 * constructor
	 */
	RubensWithKOm(char *name) : RubensWithModels(name), JoeWithModels(name), UgpWithCvCompFlow(name) {
		classID = "RubensWithKOm";
		if(mpi_rank==0)
			cout<<classID<<"()"<<endl;
...
int main(int argc, char *argv[]) {
	...
	sprintf(inputFileName, "Rubens.in");
	...
	try {
		...
		JoeWithModels *rubens;

		switch (run) {
			case 1:  rubens = new RubensWithKOm(inputFileName); 	break;
			default:
				if (mpi_rank == 0)
					cerr << "WARNING: run number not available!" << endl;
				rubens = new RubensWithKOm(inputFileName);
		}
