...
public:
	...
	virtual ~RansCombBinaryMixing() {
		if (mpi_rank == 0)
			cout << endl << "~RansCombBinaryMixing()" << endl << endl;
...
public:
	...
	virtual void initialHookScalarRansCombModel() {
		if(mpi_rank==0)
			cout<<"RansCombBinaryMixing::initialHookScalarRansCombModel()"<<endl;
	...
	virtual void ComputeBCProperties_T(FaZone *zone) {
		...
		for (int ifa = zone->ifa_f; ifa <= zone->ifa_l; ifa++) {
			int icv1 = cvofa[ifa][1];
			ComputeProperties_vis(mul_fa[ifa],lamOcp_fa[ifa],temp[icv1],zM_fa[icv1]);
		}
	}
...
//		p = rho * R * T;
	...
	void ComputeProperties_vis(double &mu, double &lamOcp, double T, double zm) {
		if (mu_ref > 0.0) {
			//mu = calcMuLam(T);
