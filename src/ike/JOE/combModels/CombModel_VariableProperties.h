...
public:
  ...
  virtual void calcThermoProp_e(double &T, double &p, double &R, double &gam, double &rho, double &e, double *Scal, int nScal, int dum1, int dum2)
  {
    // Read and interpolate species mass fractions Yk from chemistry table 
    myMixture.GetSpeciesMassFraction();
    R = myMixture.GetMixR_o_M();
  ...
  virtual void ComputeBCProperties_T(FaZone *zone)
  {
    for (int index = 0; index < zone->faVec.size(); ++index) {
      ...
      int icv1 = cvofa[ifa][1];
      ComputeProperties_T(enthalpy[icv1],RoM[icv1],gamma[icv1],temp[icv1]);
    }
     if (mu_ref > 0.0)
    {
  ...
  virtual void ComputeBCProperties_H(FaZone *zone)
  {
    for (int index = 0; index < zone->faVec.size(); ++index) {
      int ifa = zone->faVec[index];
      int icv1 = cvofa[ifa][1];
      ComputeProperties_H(temp[icv1],RoM[icv1],gamma[icv1],enthalpy[icv1]);
    }
   ...
   */
  void ComputeProperties_T(double &h, double &RoM, double &gam, double T)
  {
   ...
   */
  void ComputeProperties_H(double &T, double &RoM, double &gam, double h)
  {
    RoM = myMixture.GetMixR_o_M();
