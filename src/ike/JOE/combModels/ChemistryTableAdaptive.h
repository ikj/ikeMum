 ...
 *  i.e., the first 2 dimensions are simple vectors and the third dimension values depends on the 
 *  first 2 coordinates, i.e., x3 = x3(i,j,k).
 * 
...
#ifndef CHEMISTRYTABLEADAPTIVE_H
#define CHEMISTRYTABLEADAPTIVE_H
 ...
 *  Lookup values based on 3 dimensions by interpolating structured adaptive table
 *  using simple linear interpolation 
 *  
 *  Class assumes a structured adaptive table: x1=x1(i), x2=x2(j), x3=x3(i,j,k).
 */
class ChemtableAdaptive
{
private:
  ...
  double  *x1, *x2     ;                ///< Axis coordinates of the table (Zmean, Zvar)
  double  ***x3;                        ///< Axis coordinates of the table (chi/progress variable), function of x1 and x2
...
public:
  ...
  double GetChemtableCoordinate3(int i1, int i2, int i3) {return x3[i3][i2][i1];}
  ...
  void Load(ParamMap *myInputPtr)
  {
    ...
    getMem3D(&x3, 0, n3-1, 0, n2-1, 0, n1-1, "Chemtable::Load x3", true);
    ...
    for (k=0; k<n3; k++)
      for (j=0; j<n2; j++)
        for (i=0; i<n1; i++)
        {
          dum = fread(&x3[k][j][i], sizeof(double), 1, inFp);
          if (dum != 1)
          {
    ...
    for (k=0; k<n3; k++)
      for (j=0; j<n2; j++)
        for (i=0; i<n1; i++)
        {
          ...
          if (dum != 1)
          {
            cerr << "### Error reading file: mask -> " << i << " " << j << " " << k << " ###" << endl;
            throw(-1);
          }
        }
    if (!(is_iMask))
    {
      freeMem3D(iMask, 0, n3 - 1, 0, n2 - 1, 0, n1 - 1);   // iMask not used, save memory 
      iMask = NULL;
    }
    ...
    for (l = 0; l < nvar; l++)
    {
      ...
      {
        cout << "### Variable " << key << " already in myTableVar in loop ####" << endl;
      }
    }

    /* Table data */
    getMem4D(&Data, 0, nvar-1, 0, n3-1, 0, n2-1, 0, n1-1, "Chemtable::Load Data", false);
    for (l=0; l<nvar; l++)
      for (k=0; k<n3; k++)
        for (j=0; j<n2; j++)
          for (i=0; i<n1; i++)
    ...
    if (mpi_rank == 0)
    {
      cout << "Chemistry table is structured adaptive" << endl;
      if (is_iMask)
      ...
      GetArrayInfo(x3, n3, n2, n1, minval, maxval, mi3, mi2, mi1, ma3, ma2, ma1);
      cout.width(10);
      cout << "x1 = Zmean:  " << "\t min=" << MinVal(x1, n1) << "\t\t max=" << MaxVal(x1, n1) << endl;      
      cout.width(10);
      cout << "x2 = Zvar:   " << "\t min=" << MinVal(x2, n2) << "\t\t max=" << MaxVal(x2, n2) << endl;      
      cout.width(10);
      cout << "x3 = Prog:   " << "\t min=" << minval         << "\t\t max=" << maxval         << endl;
      
      cout << endl << "and contains " << nvar << " variables: " << endl;
      
      for (l = 0; l < nvar; l++)
      {
        strcpy(buffer_m, &VarNames[l][0]);
        GetArrayInfo(Data[l], n3, n2, n1, minval, maxval, mi3, mi2, mi1, ma3, ma2, ma1);
        cout.width(3);
        cout << l << ". ";
        cout.width(10);
        cout << buffer_m;
  ...
  void Unload()
  {
    ...
    if (x3 != NULL)       {freeMem3D(x3, 0, n3-1, 0, n2-1, 0, n1-1);                x3 = NULL;}
  ...
  void SetCoordinates(double A1, double A2, double A3)
  {
    ...
    // Third direction is different since x3 depends on x1 and x2
    // Pk is the x3 coordinate at position x1 and x2 on flamelet k
    double P0, P1;
    
    P0 =        interp.wj  * (       interp.wi  * x3[0][interp.jj]  [interp.ii] +
                              (1.0 - interp.wi) * x3[0][interp.jj]  [interp.ii+1]) +
         (1.0 - interp.wj) * (       interp.wi  * x3[0][interp.jj+1][interp.ii] + 
                              (1.0 - interp.wi) * x3[0][interp.jj+1][interp.ii+1]);
    ...
    else
    {
      for (k=1; k<n3; k++)
      {
        P1 =        interp.wj  * (       interp.wi  * x3[k][interp.jj]  [interp.ii] + 
                                  (1.0 - interp.wi) * x3[k][interp.jj]  [interp.ii+1]) +
             (1.0 - interp.wj) * (       interp.wi  * x3[k][interp.jj+1][interp.ii] + 
                                  (1.0 - interp.wi) * x3[k][interp.jj+1][interp.ii+1]);
        if (A3 <= P1)
        {
          interp.kk = k - 1;
          if (P1 == P0)
            ...
            interp.wk = (P1 - A3) / (P1 - P0);
          break;
        }
        P0 = P1;
        if (k == n3 - 1)    // i.e., A3 > x3_max  =>  clipping
        {
          interp.kk = n3 - 2;
          interp.wk = 0.0;          
        }
