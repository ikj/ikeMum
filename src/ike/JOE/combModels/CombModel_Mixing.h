...
//    eq->relax = getDoubleParam("RELAX_ZMean", "0.5");
...
//        lamOcp_fa[ifa] = myMixture.GetMixLambda() / cp;        
   ...
   */
  void ComputeProperties_T(double &h, double &RoM, double &gam, double T, double zm, double zv, double chi)
  {
   ...
   void ComputeProperties_vis(double &mu, double &lamOcp, double zm, double T)
  {
    // Read and interpolate species mass fractions Yk from linear mixing rules
    myMixture.GetSpeciesMassFraction(zm);
    double RoM = myMixture.GetMixR_o_M();
    double h = myMixture.ComputeMixEnthalpy(T);
    double cp = myMixture.ComputeMixCp(T);
    double gam = cp / (cp - RoM);
   ...
   *  \param[in]  chi Scalar dissipation rate.
   */
  void ComputeProperties_H(double &T, double &RoM, double &gam, double h, double zm, double zv, double chi)
  {
    // Read and interpolate species mass fractions Yk from linear mixing rules
    myMixture.GetSpeciesMassFraction(zm);
    RoM = myMixture.GetMixR_o_M();
