...
/* Jeff Fike TODO:  Set defaults for MUREF_1, etc.
*/
...
//    dRoM_0 = getDoubleParam("NON_LIN_MIX_D_R", "0.0");
  ...
  muref_1 = getDoubleParam("MUREF_1", "1.0");
  muref_2 = getDoubleParam("MUREF_2", "1.0");
  ...
  virtual void ComputeBCProperties_T(FaZone *zone)
  {
    ...
    if (mu_ref > 0.0)
    {
      ...
      int icv1 = cvofa[ifa][1];
      ComputeProperties_vis(mul_fa[ifa],lamOcp_fa[ifa],zM_fa[icv1],temp[icv1]);
      }
    }
  }
...
/*! \brief Compute for a given temperature the properties of the mixture.
  ...
  void ComputeProperties_vis(double &mu, double &lamOcp, double zm, double T)
  {
      //mu = calcMuLam(T);
      ...
      mu = (mu_1*zm*pow(MolMass_1,0.5)+mu_2*(1-zm)*pow(MolMass_2,0.5))/(zm*pow(MolMass_1,0.5)+(1-zm)*pow(MolMass_2,0.5));
      lamOcp = mu / Pr;
  }
  ...
  using UgpWithCvCompFlow::calcMuLam;
  virtual double calcMuLam(int icv)
  {
      ...
      double mu_1 = muref_1*pow((T/SL_Tref_1),1.5)*((SL_Tref_1+SL_Sref_1)/(T+SL_Sref_1));
      double mu_2 = muref_2*pow((T/SL_Tref_2),1.5)*((SL_Tref_2+SL_Sref_2)/(T+SL_Sref_2));
      double mu = (mu_1*zm*pow(MolMass_1,0.5)+mu_2*(1-zm)*pow(MolMass_2,0.5))/(zm*pow(MolMass_1,0.5)+(1-zm)*pow(MolMass_2,0.5));
    return mu;
  }
