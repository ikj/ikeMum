/*
 * CombModel_TripletMixing_ik.h
 *
 *  Created on: Jun 11, 2014
 *      Author: ikj
 */
// Original code = CombModel_BinaryMixing_new.h

#ifndef COMBMODEL_TRIPLETMIXING_IK_H_
#define COMBMODEL_TRIPLETMIXING_IK_H_


#define YO2_DEF    yO2 = YO2_PURE * (PHI*yfuel/YFUEL_PURE - (PHI+1.0)*zm + 1.0 );
#define ROM_DEF    R   = yfuel * RoM_1 + (1.0 - yfuel - yO2) * RoM_2 + yO2 * RoM_3;
#define CP_GAM_DEF cp  = yfuel * Cp_1  + (1.0 - yfuel - yO2) * Cp_2 + yO2 * Cp_3;   gam = cp / (cp - R);
...
// #############################################################################################
// ------                                                                                 ------
// ----                           RansCombTripletMixing                                    ----
// ------                                                                                 ------
// #############################################################################################

/*! \brief Triplet Mixing model based on mixture fraction.
 *
 *  Three species: Fuel, Oxygen, and other products (regarded as one species)
 *  The species mass fractions Species::Yk are functions of the mean scalar mixture fraction (Zmean)
 ...
 *
 *    We have two transport equations: one for rho*Yf and the other for rho*ZMean.
 *    1) Amount of fuel (rho*Yf) can be obtained by the transport equation of Yf.
 *    2) Then amount of oxygen (rho*Yo) can be calculated using Yf and Zmean:
 *                            Yf
 *         Yo = Yo,0 * ( phi*---- - (phi+1)*z + 1 )
 *                           Yf,0
 *    3) Finally, Yp = 1 - Yf - Yo.
 *
 *  Index: 1st = Fuel (H2)
 *         2nd = Product (N2, H20, etc. -- For simplicity, sometimes it includes even O2 --- I know, this assumption is not correct)
 *         3rd = Oxygen (O2)
 *
 ...
 *
 *  Fuel reaction rate is not included in this class: You may calculate it with a simple Arrhenius chemistry or Eddy dissipation Concept (EDC).
 */

class RansCombTripletMixing: public RansCombBase
{
public:
	// member variables
...
public:
	...
	RansCombTripletMixing() {
		if (mpi_rank == 0)
			cout << "RansCombTripletMixing()" << endl;
...
//		eq2->upperBound = 1.0;
	...
	virtual ~RansCombTripletMixing() {
		if (mpi_rank == 0)
			cout << endl << "~RansCombTripletMixing()" << endl << endl;
...
public:
	...
	virtual void initialHookScalarRansCombModel() {
		if(mpi_rank==0)
			cout<<"RansCombTripletMixing::initialHookScalarRansCombModel()"<<endl;
		...
		if (mpi_rank == 0)
			cout << endl << "***** Triplet Mixing initialized *****" << endl << endl;
	}
	...
	virtual void calcStateVariables() {
		double yfuel, yO2, zm, R, E, cp, gam, h, T;
		double kinecv = 0.0;
	...
	virtual void pressureDerivativeHookScalarRansComb() {
		cout<<"ERROR in RansCombTripletMixing::pressureDerivativeHookScalarRansComb(): This method has not been implemented!!"<<endl;
		throw(-1);
...
//		}
	...
	virtual void ComputeBCProperties_T(FaZone *zone) {
		...
		for (int index = 0; index < zone->faVec.size(); ++index) {
			...
			ComputeProperties_T(enthalpy[icv1], RoM[icv1], gamma[icv1], temp[icv1], yfuel_fa[icv1], zM_fa[icv1], 0.0, 0.0);
		}
		for (int ifa = zone->ifa_f; ifa <= zone->ifa_l; ifa++) {
			int icv1 = cvofa[ifa][1];
			ComputeProperties_vis(mul_fa[ifa], lamOcp_fa[ifa], temp[icv1], yfuel_fa[icv1], zM_fa[icv1]);
		}
	}
	...
	virtual void ComputeBCProperties_H(FaZone *zone) {
		...
		for (int index = 0; index < zone->faVec.size(); ++index) {
			int ifa = zone->faVec[index];
			int icv1 = cvofa[ifa][1];
			ComputeProperties_H(temp[icv1], RoM[icv1], gamma[icv1], enthalpy[icv1], yfuel_fa[icv1], zM_fa[icv1], 0.0, 0.0);
		}
		for (int ifa = zone->ifa_f; ifa <= zone->ifa_l; ifa++) {
			int icv1 = cvofa[ifa][1];
			ComputeProperties_vis(mul_fa[ifa], lamOcp_fa[ifa], temp[icv1], yfuel_fa[icv1], zM_fa[icv1]);
	 ...
	 */
	void ComputeProperties_T(double &h, double &R, double &gam, double T, double yfuel, double zm, double zv, double chi) {
		double cp, yO2;
		YO2_DEF
	...
	void ComputeProperties_vis(double &mu, double &lamOcp, double T, double yfuel, double zm) {
		if (mu_ref > 0.0) {
			double yO2;
			YO2_DEF
			...
			double MolMass_tot = 1.0 / (yfuel/MolMass_1 + (1.0-yfuel-yO2)/MolMass_2  + yO2/MolMass_3);
			double x1 = MolMass_tot / MolMass_1 * yfuel;   // Mole fraction of the fuel
			double x2 = MolMass_tot / MolMass_2 * (1.0 - yfuel - yO2);
			double x3 = 1.0 - x1 - x2;   // Mole fraction of oxygen
			mu = (mu_1*x1*sqrt(MolMass_1) + mu_2*x2*sqrt(MolMass_2) + mu_3*x3*sqrt(MolMass_3)) / (x1*sqrt(MolMass_1) + x2*sqrt(MolMass_2) + x3*sqrt(MolMass_3));
	 ...
	 *  \param[in]  chi Scalar dissipation rate. !!!
	 */
	void ComputeProperties_H(double &T, double &R, double &gam, double h, double yfuel, double zm, double zv, double chi) {
		double yO2;
		YO2_DEF
	...
	virtual void diffusivityHookScalarRansComb(const string &name) {
		...
		if (name.compare("ZMean")==0 || name.compare("YFuel")==0) {
	...
	double calcMuLam(int icv) {
		...
		double mu = (mu_1*yfuel*pow(MolMass_1,0.5)+mu_2*(1-yfuel-yO2)*pow(MolMass_2,0.5)+mu_3*yfuel*pow(MolMass_3,0.5)) / (yfuel*pow(MolMass_1,0.5)+(1-yfuel-yO2)*pow(MolMass_2,0.5)+yO2*pow(MolMass_3,0.5));
		return mu;
	}
};
...
#endif /* COMBMODEL_TRIPLETMIXING_IK_H_ */
