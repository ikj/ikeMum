...
public:
  ...
  virtual void calcStateVariables_AD(adouble *rho, adouble (*rhou)[3], adouble *rhoE)
  {
    ...
    for (int icv = 0; icv < ncv_gg; icv++)
    {
      ...
      ZMean_i = ZMean[icv].value();
      ZVar_i  = ZVar[icv].value();
      CMean_i = CMean[icv].value();
      myChemTable.LookupCoeff(RoM_o, T0, E0, GAMMA0, AGAMMA, MU0, AMU, LOC0, ALOC, CmeanS_o, ZMean_i, ZVar_i, CMean_i);
      
      CmeanSource_AD[icv]=CmeanS_o;
      RoM[icv]           =RoM_o;
  ...
  virtual void calcThermoProp_T_AD(adouble &p, adouble &h, adouble &R, adouble &gam, adouble &rho, adouble &T, adouble *Scal, int nScal)
  {
    ...
    myChemTable.LookupSelectedCoeff(RoM_o, T0, E0, GAMMA0, AGAMMA, zm.value(), zv.value(), cm.value());
    R=RoM_o;
    if (AGAMMA == 0.0)
  ...
  void ComputeProperties_T_AD(adouble &h, adouble &RoM, adouble &gam, adouble T, adouble zm, adouble zv, adouble cm)
  {
    ...
    myChemTable.LookupCoeff(R, T0, E0, GAMMA0, AGAMMA, MU0, AMU, LOC0, ALOC, dummy, zm.value(), zv.value(), cm.value());
    RoM=R;
    if (AGAMMA == 0.0)
  ...
  void ComputeProperties_H_AD(adouble &T, adouble &RoM, adouble &gam, adouble h, adouble zm, adouble zv, adouble cm)
  {
    ...
    adouble SolveNewtonTemperature_H_AD(adouble R, double t0, double e0, double gamma0, double agamma, adouble h, adouble Tguess)
  {
    int    itermax = 50, iter;
