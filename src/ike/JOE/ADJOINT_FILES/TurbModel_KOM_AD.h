...
public:   // member vars
	ADvector<adouble>* grad_kine;
	ADscalar<adouble>* kine_diff;

	ADscalar<adouble>* omega;
	ADvector<adouble>* grad_omega;
	ADscalar<adouble>* omega_diff;

public:   // member functions
	virtual void initialize_turb_adjoint()
	{
		if(mpi_rank==0)
			cout<<"RansTurbKOm_AD::initialize_turb_adjoint(): This function is doning nothing!"<<endl;
	}

	virtual void copy_turb_adjoint()
	{
		if(mpi_rank==0)
			cout<<"RansTurbKOm_AD::copy_turb_adjoint(): This function is doning nothing!"<<endl;
	}

	virtual void initialHookScalarRansTurbModel_AD()
	{
		// connect pointers: This will cause a waste of computational time, but I don't know how to go around right now
