...
public:
  RansTurbV2F()
  {
    ...
    eq->phiZero = 1.0e-8;
    eq->phiMaxiter = 500;
    eq->lowerBound = 0.66667e-10;
    //eq->lowerBound = 1.e-10;
    eq->upperBound = 1.0e10;
    eq->turbSchmidtNumber = SIG_K;
...
public:
                                  ...
                                  const double &str, const double &nu, int realizable)
  {
    ...
    if (realizable)
      TimeScale = min(TimeScale, kine/(pow(3.0, 0.5)*v2*C_MU*str));
    return TimeScale;
  }
  ...
  virtual void sourceHookScalarRansTurb(double *rhs, double *A, const string &name, int flagImplicit)
  {
    ...
    if (name == "f")      //
    { //if(mpi_rank==0) cout<<"REALIZABLE F"<<endl;
      for (int icv = 0; icv < ncv; icv++)
      {

      double TS  = calcTurbTimeScale(kine[icv], eps[icv], v2[icv], strMag[icv], calcMuLam(icv)/rho[icv], 0);

      double LS = max(pow(kine[icv], 1.5)/eps[icv], CETA*pow(calcMuLam(icv)/rho[icv],0.75)/pow(eps[icv],0.25));

      //LS = min(LS, pow(kine[icv], 1.5)/(sqrt(3.)*v2[icv]*C_MU*strMag[icv]));
