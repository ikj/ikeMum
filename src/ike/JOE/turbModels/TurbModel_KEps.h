  ...
  virtual void calcRansTurbViscMuet()
  {
    ...
    for (int ifa=nfa_b; ifa<nfa; ifa++)
    {
      ...
      if (realizable == 1)
      {
        ...
        double cmu = min(C_MU, sqrt(C_MU)/max(s,1.e-12));
      ...
      if (zoneIsWall(zone->getName()))
        ...
        for (int ifa=zone->ifa_f; ifa<=zone->ifa_l; ifa++)
        {
          ...
          if (realizable == 1)
          {
            ...
            double cmu = min(C_MU, sqrt(C_MU)/max(s,1.e-12));
            mut_fa[ifa] = min(cmu*rho[icv1]*kine[icv1]*kine[icv1]/eps[icv1], 1000.0);    // zero order extrapolation for others
          }
          else
            mut_fa[ifa] = min(C_MU*rho[icv1]*kine[icv1]*kine[icv1]/eps[icv1], 1000.0);    // zero order extrapolation for others
        }
    }
