...
public:   // member functions
  ...
  virtual void initialHookScalarRansTurbModel()
  {
    ...
    // connect pointers
    ScalarTranspEq *eq;
    eq = getScalarTransportData("gam");   gam = eq->phi;
    eq = getScalarTransportData("ReT");   ReT = eq->phi;
  ...
  virtual void sourceHookScalarRansTurb(double *rhs, double *A, const string &name, int flagImplicit)
  {
    if (name == "kine")
    {
      for (int icv=0; icv<ncv; icv++)
      {
        ...
        double src =   gam_eff*calcTurbProd(icv, SST_limitPK)
                     - faktDestr*betaStar*rho[icv]*omega[icv]*kine[icv];
    ...
    if (name == "gam")
    {
      ...
      for (int icv=0; icv<ncv; icv++)
      {
        ...
        double nsigma_mach = max(1.0 - (0.14 + 0.7*fakt_nSigma)*Mais[icv], 0.05);
...
/*
  ...
  {
    TURB_MOD_FOR_TRANS::boundaryHookScalarRansTurb(phi, zone, name);
      ...
      if (getParam(param, zone->getName()))
        if (name == "ReT")
        {
          for (int ifa=zone->ifa_f; ifa<=zone->ifa_l; ifa++)
          {
            ...
            double U     = max(sqrt(vecDotVec3d(vel[icv1], vel[icv1])), 1.0e-6);
            double TuLoc = max(100.0*sqrt(2.0*kine[icv1]/3.0)/U, 0.027);
