...
public:   // member functions
  ...
  virtual void initialHookScalarRansTurbModel()
  {
    ...
    // connect pointers 
    ScalarTranspEq *eq = getScalarTransportData("nuSA");      nuSA = eq->phi;
    ...
    if (zone->getKind() == FA_ZONE_BOUNDARY)
    {
      if (zoneIsWall(zone->getName()))
        ...
        for (int ifa=zone->ifa_f; ifa<=zone->ifa_l; ifa++)
        {
          ...
          double muSA  = rho[icv1]*nuSA[icv1];
          //double mul_icv0 = InterpolateAtCellCenterFromFaceValues(mul_fa, icv0);
          double chi_3 = pow(muSA/mul_fa[ifa], 3.0);
          double fv1   = chi_3/(chi_3 + cv1_3);
    ...
    if (name == "nuSA")
    {
      ...
      if (zone->getKind() == FA_ZONE_BOUNDARY)
      {
        if (zoneIsWall(zone->getName()))
          ...
          {
            int icv1 = cvofa[ifa][1];
            eq->diff[ifa] = invSigma*(mul_fa[ifa] + rho[icv1]*nuSA[icv1]);
          }
      }
    }
  }
