...

  {
    ...
    for (int ifa = 0; ifa<nfa; ifa++)
    {
    	...
    	if (cvofa[ifa][1]>=0)
    	{
    		...
    		if(fa_kind_table[fa_flag[ifa]]!=FA_ZONE_INTERNAL) {
    			cout<<"ERROR in Ugp::finalizeReadRestart(): fa_flag is not valid, ifa="<<ifa<<", fa_flag[ifa]="<<fa_flag[ifa]<<endl;

//    			int nnof = noofa_i[ifa+1]-noofa_i[ifa];
//    			double x_fa[3] = {0.0, 0.0, 0.0};
//    			for(int nof = noofa_i[ifa]; nof < noofa_i[ifa] + nnof; ++nof) {
//    				int ino = noofa_v[nof] - noora[mpi_rank];
//    				cout<<"ino="<<ino<<endl;
//
//    				for(int i=0; i<3; ++i)
//    					x_fa[i] += x_no[ino][i];
...
//    			for(int i=0; i<3; ++i)
//    				x_fa[i] /= double(nnof);
//    			cout<<", x_fa="<<x_fa[0]<<","<<x_fa[1]<<","<<x_fa[2]<<endl;
    		}
//    		assert(fa_kind_table[fa_flag[ifa]]==FA_ZONE_INTERNAL);
    	}
    	...
    	else
    	{
    		...
    		assert((fa_kind_table[fa_flag[ifa]]>=FA_ZONE_PERIODIC_FIRST)&&(fa_kind_table[fa_flag[ifa]]<=FA_ZONE_PERIODIC_LAST));
