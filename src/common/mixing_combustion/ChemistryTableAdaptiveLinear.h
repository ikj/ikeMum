...
public:  
  ...
  // Compute weights
  {
    ...
    for (int j = 0; j < 2; j++)
    {
      f[j] = 0.0;
      for (int i = 0; i < 2; i++)
        f[j] += weight[0][i] * x3[interp->index[0]+i][interp->index[1]+j][1];
      P0 += weight[1][j] * f[j];
    }
    ...
    else
    {
      for (int k=2; k<=n3; k++)
      {
        ...
        for (int j = 0; j < 2; j++)
        {
          f[j] = 0.0;
          for (int i = 0; i < 2; i++)
            f[j] += weight[0][i] * x3[interp->index[0]+i][interp->index[1]+j][k];
          P1 += weight[1][j] * f[j];
        }
        if (A3 <= P1)
        {
