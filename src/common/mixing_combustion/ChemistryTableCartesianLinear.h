...
public:  
  ...
  void SetCoordinates_AD(adouble A1, adouble A2, adouble A3, adouble (*weight)[2])
  // Find index ii such that x1[ii] <= A1 < x1[ii+1]
  // Compute weights
  {
    double x0, xp1, A1v, A2v,A3v;
    A1v = A1.value();
    A2v = A2.value();
    A3v = A3.value();
    
    // First direction
    BinarySearch(interp->index[0], x1, 1, n1, A1v);
    
    x0  = x1[interp->index[0]];
    xp1 = x1[interp->index[0] + 1];
    
    Compute_Weights_AD(A1, x0, xp1, weight[0]);
    
    // Second direction
    BinarySearch(interp->index[1], x2, 1, n2, A2v);
    
    x0  = x2[interp->index[1]];
    xp1 = x2[interp->index[1] + 1];
    
    Compute_Weights_AD(A2, x0, xp1, weight[1]);
    
    // Third direction
    BinarySearch(interp->index[2], x3, 1, n3, A3v);
    
    x0  = x3[interp->index[2]];
    xp1 = x3[interp->index[2] + 1];
    
    Compute_Weights_AD(A3, x0, xp1, weight[2]);
  ...
  void Compute_Weights_AD(adouble x, double x0, double xp1, adouble *wi)
  {
    double xp1x0;
  ...
  adouble Lookup_AD(adouble A1, adouble A2, adouble A3, string tag)
  {
    adouble val, g[4], f[4];
 ...
 /***********************************************************************************************************/
  ...
  adouble Lookup_AD(adouble A1, adouble A2, adouble A3, int ll)
  {
    adouble val, g[4], f[4];
...
//  }
   ...
   *
   * Output: 10 variables from the table (RoM, T0, E0, Gam0, a_Gam, mu0, a_mu, lamOcp0, a_lamOcp, src_prog)
